...
# Modify the 'store-command' route to store the commands and save to file
...
# Modify the 'open-task' route to check against stored commands
@app.route('/open-task', methods=['POST'])
def open_tabs():
    audio = get_voice()
    text = convert_audio_to_text(audio)
    if text.lower() in stored_commands:
        webbrowser.open(stored_commands[text.lower()])
